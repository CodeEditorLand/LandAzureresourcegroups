...
export namespace apiUtils {
	 ...
	 */
	export async function getAzureExtensionApi<T extends AzureExtensionApi>(
		context: vscode.ExtensionContext,
		extensionId: string,
		apiVersionRange: string,
		options?: GetApiOptions,
	): Promise<T> {
		const apiProvider: AzureExtensionApiProvider | undefined =
			await getExtensionExports(extensionId);
	 ...
	 */
	export async function getExtensionExports<T>(
		extensionId: string,
	): Promise<T | undefined> {
