...
export abstract class ResourceTreeDataProviderBase
	...
	implements vscode.TreeDataProvider<ResourceGroupsItem>
{
	...
	constructor(
		...
		onDidChangeBranchTreeData: vscode.Event<
			void | ResourceModelBase | ResourceModelBase[] | null | undefined
		>,
		onDidChangeResource: vscode.Event<ResourceBase | undefined>,
		onRefresh: vscode.Event<
			void | ResourceGroupsItem | ResourceGroupsItem[] | null | undefined
		>,
		...
		this.branchTreeDataChangeSubscription = onDidChangeBranchTreeData((e) =>
			this.notifyTreeDataChanged(e),
		);

		this.refreshSubscription = onRefresh((e) =>
	...
	notifyTreeDataChanged(
		data: void | ResourceModelBase | ResourceModelBase[] | null | undefined,
	): void {
		...
		if (!!data) {
			...
			const branchItems: ResourceModelBase[] = Array.isArray(data)
				? data
				: [data];
	...
	getParent(
		element: ResourceGroupsItem,
	): vscode.ProviderResult<ResourceGroupsItem> {
	...
	async findItemById(id: string): Promise<ResourceGroupsItem | undefined> {
		...
		outerLoop: while (true) {
			const children: ResourceGroupsItem[] | null | undefined =
	...
	protected isAncestorOf(element: ResourceGroupsItem, id: string): boolean {
		...
		const elementId =
			element.id
				.replace(/\/accounts\/[^/]+\/tenants\/[^/]+\//i, "/")
				.toLowerCase() + "/";
		return id.toLowerCase().startsWith(elementId);
	}

	protected abstract onGetChildren(
		element?: ResourceGroupsItem | undefined,
	): Promise<ResourceGroupsItem[] | null | undefined>;
}
