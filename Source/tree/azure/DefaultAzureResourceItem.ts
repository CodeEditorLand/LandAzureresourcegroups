...
export class DefaultAzureResourceItem implements ResourceGroupsItem {
	...
	getChildren(): Promise<ResourceGroupsItem[] | undefined> {
		if (
			...
			!this.resourceTypeExtension.isInstalled()
		) {
			return Promise.resolve([
				new GenericItem(
					localize(
						"installExtensionToEnableFeatures",
						"Install extension to enable additional features...",
					),
					{
	...
	getTreeItem(): vscode.TreeItem | Thenable<vscode.TreeItem> {
		const isResourceTypeExtensionInstalled =
			this.resourceTypeExtension?.isInstalled();

		const treeItem = new vscode.TreeItem(
			this.resource.name ?? "Unnamed Resource",
			isResourceTypeExtensionInstalled === false
