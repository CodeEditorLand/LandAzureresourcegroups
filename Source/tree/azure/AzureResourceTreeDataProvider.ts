...
} from "../../commands/accounts/selectSubscriptions";
...
import {
	getAzureSubscriptionProvider,
	OnGetChildrenBase,
} from "../OnGetChildrenBase";
...
export class AzureResourceTreeDataProvider extends AzureResourceTreeDataProviderBase {
	...
	constructor(
			...
			async (
				...
				if (
					e.affectsConfiguration(
						`${ext.prefix}.${showHiddenTypesSettingKey}`,
					)
				) {
		...
		this.groupingChangeSubscription =
			this.resourceGroupingManager.onDidChangeGrouping(() =>
				this.notifyTreeDataChanged(),
			);
	}
	...
	async onGetChildren(
		...
		if (element) {
			...
			const subscriptionProvider =
				await getAzureSubscriptionProvider(this);
			// When a user is signed in 'OnGetChildrenBase' will return no children
			const children: ResourceGroupsItem[] = await OnGetChildrenBase(
			...
			if (children.length === 0) {
				...
				if (
					...
					const tenantFiltedSubcriptions =
						getTenantFilteredSubscriptions(subscriptions);
					if (tenantFiltedSubcriptions) {
						return tenantFiltedSubcriptions.map((subscription) => {
								...
								return new SubscriptionItem(
									{
										...
										refresh: (item) =>
											this.notifyTreeDataChanged(item),
									},
