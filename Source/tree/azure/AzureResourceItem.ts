...
import {
	BranchDataItemOptions,
	BranchDataItemWrapper,
} from "../BranchDataItemWrapper";
import { ResourceGroupsItem } from "../ResourceGroupsItem";

export class AzureResourceItem<
	T extends AzureResource,
> extends BranchDataItemWrapper {
	constructor(
...
export type ResourceItemFactory<T extends AzureResource> = (
	resource: T,
	branchItem: ResourceModelBase,
	branchDataProvider: BranchDataProvider<ResourceBase, ResourceModelBase>,
...
export function createResourceItemFactory<T extends AzureResource>(
	itemCache: BranchDataItemCache
): ResourceItemFactory<T> {
	return (resource, branchItem, branchDataProvider, parent, options) =>
		itemCache.createOrGetItem(
			branchItem,
			() =>
				new AzureResourceItem(
					resource,
					branchItem,
					branchDataProvider,
					itemCache,
