...
export class GroupingItem implements ResourceGroupsItem {
	...
	async getChildren(): Promise<ResourceGroupsItem[] | undefined> {
		const sortedResources = this.getResourcesToDisplay(this.resources).sort(
			(a, b) => {
				if (a.subscription !== b.subscription) {
					return a.subscription.name.localeCompare(
						b.subscription.name,
					);
				}
		...
		const subscriptionGroupingMap = new Map<
			AzureSubscription,
			AzureResource[]
		>();
		...
		const resourceItems = await Promise.all(
			Array.from(subscriptionGroupingMap.keys()).map(
				async (subscription) => {
					const items: ResourceGroupsItem[] = [];

					if (
						!this.options?.hideSeparators &&
						subscriptionGroupingMap.size > 1
					) {
						items.push(
							new GenericItem("", {
					...
					await Promise.allSettled(
						(subscriptionGroupingMap.get(subscription) ?? []).map(
							async (resource): Promise<void> => {
								try {
									...
									const options: BranchDataItemOptions = {
										...
										portalUrl:
											resourceItem.portalUrl ??
											createPortalUrl(
												resource.subscription,
												resource.id,
											),
										viewProperties:
											resourceItem.viewProperties ?? {
												label: resource.name,
												data: resource.raw,
											},
									};
									...
									ext.outputChannel.appendLog(
										localize(
											"errorResolving",
											"Error resolving resource item for {0}: {1}",
											resource.id,
											parsedError.message,
										),
									);
...
export type GroupingItemFactory = (
	options: GroupingItemOptions,
) => GroupingItem;

export function createGroupingItemFactory(
	resourceItemFactory: ResourceItemFactory<AzureResource>,
	branchDataProviderFactory: BranchDataProviderFactory,
	onDidChangeBranchDataProviders: vscode.Event<AzExtResourceType>,
	defaultOptions?: GroupingItemDisplayOptions,
): GroupingItemFactory {
	return ({
		context,
		contextValues,
		...
		new GroupingItem(
			{
				...
				resources,
				parent,
				displayOptions: { ...defaultOptions, ...displayOptions },
			},
			{
				resourceItemFactory,
				branchDataProviderFactory,
				onDidChangeBranchDataProviders,
			},
		);
}
