...
function groupBy({
	...
	return Object.entries(map).map(([key, resources]) =>
		groupingItemFactory(key, resources)
	);
}
...
export class AzureResourceGroupingManager extends vscode.Disposable {
	...
	groupResources(
		parent: ResourceGroupsItem | undefined,
		context: ResourceGroupsTreeContext | undefined,
	...
	private groupByResourceType(
		...
		allResources = allResources.filter(
			(resource) =>
				resource.azureResourceType.type !==
		...
		if (!showHiddenTypes) {
			...
			allResources = allResources.filter(
				(resource) =>
					...
					supportedResourceTypes.find(
						(type) => type === resource.resourceType
					)
			);
		}
		...
		return groupBy({
			...
			groupingItemFactory: (resourceType, resources) =>
				this.groupingItemFactory.createResourceTypeGroupingItem(
					resourceType as AzExtResourceType,
					{
						resources,
						context,
						parent,
						label:
							getName(resourceType as AzExtResourceType) ??
							resourceType,
	...
	private groupByResourceGroup(
		...
		allResources.forEach((resource) =>
			...
			"microsoft.resources/resourcegroups"
				...
				: nonResourceGroups.push(resource)
		);
		...
		return groupBy({
			...
			keySelector: (resource) =>
				resource.resourceGroup?.toLowerCase() ?? unknownLabel, // TODO: Is resource group ever undefined? Should resource group be normalized on creation?
			initialGrouping,
			groupingItemFactory: (
				resourceGroupName,
				resources
				...
				return this.groupingItemFactory.createResourceGroupGroupingItem(
					nonNullValue(
						resourceGroup,
						"resourceGroup for grouping item"
					),
					{
						context,
						resources,
	...
	private groupByLocation(
		...
		return groupBy({
			...
			groupingItemFactory: (location, resources): GroupingItem =>
				this.groupingItemFactory.createLocationGroupingItem(location, {
					context,
					parent,
					resources,
					label: location,
	...
	private groupByArmTag(
		...
		return groupBy({
			...
			keySelector: (resource: AzureResource) =>
				resource.tags?.[tag] ?? ungroupedKey,
			groupingItemFactory: (tag, resources): GroupingItem =>
				this.groupingItemFactory.createGenericGroupingItem({
					parent,
					context,
					resources,
					iconPath: new vscode.ThemeIcon(
						tag !== ungroupedKey ? "tag" : "json"
					),
					label:
						tag !== ungroupedKey
							? tag
							: localize("ungrouped", "ungrouped"),
				}),
		});
	}
}
