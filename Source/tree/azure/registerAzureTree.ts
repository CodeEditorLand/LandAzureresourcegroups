...
export function registerAzureTree(
	context: vscode.ExtensionContext,
	options: RegisterAzureTreeOptions,
): AzureResourceTreeDataProvider {
	const {
		azureResourceBranchDataProviderManager,
		azureResourceProviderManager: resourceProviderManager,
		refreshEvent,
		itemCache,
	} = options;
	...
	const resourceGroupingManager = createGroupingManager(
		azureResourceBranchDataProviderManager,
		itemCache,
	);
	...
	const treeView = createTreeView("azureResourceGroups", {
		...
		findItemById: azureResourceTreeDataProvider.findItemById.bind(
			azureResourceTreeDataProvider,
...
function createGroupingManager(
	azureResourceBranchDataProviderManager: AzureResourceBranchDataProviderManager,
	itemCache: BranchDataItemCache,
	...
	const groupingItemFactory = new GroupingItemFactory({
		...
		branchDataProviderFactory: (r) =>
			azureResourceBranchDataProviderManager.getProvider(r.resourceType),
		onDidChangeBranchDataProviders:
			azureResourceBranchDataProviderManager.onChangeBranchDataProviders,
