...
export async function OnGetChildrenBase(
	subscriptionProvider: AzureSubscriptionProvider,
	tdp?: AzureResourceTreeDataProvider,
): Promise<ResourceGroupsItem[]> {
				...
				children.push(
					new GenericItem(
						...
						),
						{
							...
							commandArgs: [
								"https://aka.ms/VSCodeCreateAzureAccount",
							],
...
export async function getAzureSubscriptionProvider(
	tdp: AzureResourceTreeDataProviderBase | TenantResourceTreeDataProvider,
): Promise<AzureSubscriptionProvider> {
	...
	if (ext.testing.overrideAzureSubscriptionProvider) {
		...
		tdp.statusSubscription = vscode.authentication.onDidChangeSessions(
			(evt: vscode.AuthenticationSessionsChangeEvent) => {
				if (
					evt.provider.id === "microsoft" ||
					evt.provider.id === "microsoft-sovereign-cloud"
				) {
					if (Date.now() > tdp.nextSessionChangeMessageMinimumTime) {
						tdp.nextSessionChangeMessageMinimumTime =
							Date.now() + tdp.sessionChangeMessageInterval;
