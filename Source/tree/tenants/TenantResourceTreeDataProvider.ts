			...
			async (context: IActionContext) => {
				if (element) {
					...
					const children: ResourceGroupsItem[] =
						await OnGetChildrenBase(subscriptionProvider);
					...
					if (children.length === 0) {
						...
						context.telemetry.properties.accountCount =
							accounts.length.toString();
						for (const account of accounts) {
							const tenants = (
								await subscriptionProvider.getTenants(account)
							)
							...
							for await (const tenant of tenants) {
								const isSignedIn =
									await subscriptionProvider.isSignedIn(
										nonNullProp(tenant, "tenantId"),
										account,
									);
								tenantItems.push(
									new TenantTreeItem(tenant, account, {
										contextValue: isSignedIn
											? "tenantName"
											: "tenantNameNotSignedIn",
										checkboxState:
											!isSignedIn ||
											isTenantFilteredOut(
												nonNullProp(tenant, "tenantId"),
							...
							children.push(
								new GenericItem(
									nonNullValueAndProp(account, "label"),
									{
