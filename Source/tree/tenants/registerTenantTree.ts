...
export function registerTenantTree(
	context: vscode.ExtensionContext,
	options: RegisterTenantTreeOptions,
): TenantResourceTreeDataProvider {
	const {
	...
	const tenantResourceTreeDataProvider = new TenantResourceTreeDataProvider(
		tenantResourceBranchDataProviderManager,
		tenantResourceBranchDataProviderManager.onDidChangeTreeData,
		refreshEvent,
		tenantResourceProviderManager,
		itemCache,
	);
	...
	const treeView = createTreeView("azureTenantsView", {
		...
		treeDataProvider: wrapTreeForVSCode(
			tenantResourceTreeDataProvider,
			itemCache,
		),
		findItemById: tenantResourceTreeDataProvider.findItemById.bind(
			tenantResourceTreeDataProvider,
		) as typeof tenantResourceTreeDataProvider.findItemById,
	});
	...
	registerEvent(
		...
		async (
			...
			args: vscode.TreeCheckboxChangeEvent<TenantTreeItem>,
		) => {
...
async function updateTenantsSetting(
	context: IActionContext,
	tenants: vscode.TreeCheckboxChangeEvent<TenantTreeItem>,
) {
		...
		if (state === vscode.TreeItemCheckboxState.Unchecked) {
			...
			if (treeItem?.contextValue === "tenantNameNotSignedIn") {
				await vscode.commands.executeCommand(
					"azureTenantsView.signInToTenant",
					tenantTreeItem,
				);
				ext.actions.refreshTenantTree();
			}
			unselectedTenantsSet.delete(
				getKeyForTenant(
					tenantTreeItem.tenantId,
					tenantTreeItem.account.id,
				),
...
export function isTenantFilteredOut(
	tenantId: string,
	accountId: string,
): boolean {
