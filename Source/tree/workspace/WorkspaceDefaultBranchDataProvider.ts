...
export class WorkspaceDefaultBranchDataProvider
	implements BranchDataProvider<WorkspaceResource, WorkspaceResourceModel>
{
	getChildren(
		_element: WorkspaceResourceModel
	): vscode.ProviderResult<WorkspaceResourceModel[]> {
		return [];
	}

	getResourceItem(
		element: WorkspaceResource
	): WorkspaceResourceModel | Thenable<WorkspaceResourceModel> {
		return new WorkspaceResourceItem(element);
	}

	getTreeItem(
		element: WorkspaceResourceModel
