...
export class WorkspaceResourceTreeDataProvider extends ResourceTreeDataProviderBase {
		...
		if (element) {
			...
			if (resources.length === 0) {
				await vscode.commands.executeCommand(
					"setContext",
					"azureWorkspace.state",
					this.resourceProviderManager.hasResourceProviders
						? "noWorkspaceResources"
						: "noWorkspaceResourceProviders",
				);
			} else {
				children.push(
					...(await Promise.all(
						resources.map((resource) =>
							this.getWorkspaceItemModel(resource),
						),
					)),
				);
	...
	private async getWorkspaceItemModel(
