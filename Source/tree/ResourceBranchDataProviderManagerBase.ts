...
export abstract class ResourceBranchDataProviderManagerBase<
	TResourceType,
	TBranchDataProvider extends BranchDataProvider<
		ResourceBase,
		ResourceModelBase
	>,
> extends vscode.Disposable {
	private readonly branchDataProviderMap = new Map<
		TResourceType,
		{
	...
	addProvider(type: TResourceType, provider: TBranchDataProvider): void {
		this.branchDataProviderMap.set(type, {
...
function wrapBranchDataProvider<
	TBranchDataProvider extends BranchDataProvider<
		ResourceBase,
		ResourceModelBase
	>,
	TResourceType,
>(
	branchDataProvider: TBranchDataProvider,
	type: TResourceType
): TBranchDataProvider {
	return {
		...wrapFunctionsInTelemetry(
			{
				getChildren: branchDataProvider.getChildren.bind(
					branchDataProvider
				) as typeof branchDataProvider.getChildren,
				getTreeItem: branchDataProvider.getTreeItem.bind(
					branchDataProvider
				) as typeof branchDataProvider.getResourceItem,
			...
			},
			{
				...
				beforeHook: (context: IActionContext) => {
					context.telemetry.properties.branchDataProviderType =
						String(type);
				},
			}
		),
		onDidChangeTreeData: branchDataProvider.onDidChangeTreeData?.bind(
			branchDataProvider
		) as typeof branchDataProvider.onDidChangeTreeData,
		resolveTreeItem: branchDataProvider.resolveTreeItem?.bind(
			branchDataProvider
		) as typeof branchDataProvider.resolveTreeItem,
	} as BranchDataProvider<
		ResourceBase,
		ResourceModelBase
	> as TBranchDataProvider;
}

class NullishGetResourceItemResultError extends Error {
	constructor(result: never) {
		super(
			`Internal error: getResourceItem returned ${String(
				result
			)}. Expected a non-nullish value.`
		);
