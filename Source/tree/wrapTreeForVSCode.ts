 ...
 */
export function wrapTreeForVSCode(
	treeDataProvider: ResourceTreeDataProviderBase,
	branchItemCache: BranchDataItemCache,
): OnRefreshTreeDataProvider {
	return new OnRefreshTreeDataProvider(treeDataProvider, () =>
		branchItemCache.clear(),
	);
}
 ...
 */
class OnRefreshTreeDataProvider
	implements TreeDataProvider<ResourceGroupsItem>
{
	constructor(
	...
	): ProviderResult<ResourceGroupsItem[]> {
		if (!element) {
			this.onRefresh();
		}
