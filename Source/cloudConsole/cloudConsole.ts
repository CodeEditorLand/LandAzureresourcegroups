...
import {
	Deferred,
	delay,
	logAttemptingToReachUrlMessage,
} from "./cloudConsoleUtils";
...
import {
	...
	CloudShellStatus,
	UploadOptions,
} from "./CloudShellInternal";
import { createServer, Queue, Server } from "./ipc";
...
function getUploadFile(
	tokens: Promise<AccessTokens>,
	uris: Promise<ConsoleUris>,
): (
	this: CloudShell,
	filename: string,
	stream: ReadStream,
...
export function createCloudConsole(
	...
	osName: OSName,
	terminalProfileToken?: CancellationToken,
): CloudShellInternal {
	// eslint-disable-next-line @typescript-eslint/no-non-null-assertion
	return callWithTelemetryAndErrorHandlingSync(
		...
		(context: IActionContext) => {
			...
			const state: CloudShellInternal = {
				...
				waitForConnection,
				terminal: new Promise<Terminal>(
					(resolve, reject) =>
						(deferredTerminal = { resolve, reject }),
				),
				terminalProfile: new Promise<TerminalProfile>(
					(resolve, reject) =>
						(deferredTerminalProfile = { resolve, reject }),
				),
			...
			(async function (): Promise<any> {
					...
					async (req, res) => {
						...
						for (const message of await readJSON(req)) {
							...
							if (message.type === "poll") {
								...
								deferredInitialSize.resolve(
									message.size as Size,
								);
							} else if (message.type === "status") {
								updateStatus(
									message.status as CloudShellStatus,
								);
							}
						}
				...
				if (!(await subscriptionProvider.isSignedIn())) {
					...
					try {
						...
						serverQueue.push({
							type: "log",
							args: [
								localize("loggingIn", parseError(e).message),
							],
						});
				...
				// open terminal
				let cloudConsoleLauncherPath: string = path.join(
				...
				if (tenants.length <= 1) {
					serverQueue.push({
						type: "log",
						args: [localize("foundOneTenant", `Found 1 tenant.`)],
					});
					...
					serverQueue.push({
						...
						args: [
							localize(
								"foundTenants",
								`Found ${tenantsWithSubs.length} authenticated tenant${tenants.length > 1 ? "s" : ""}. Please use the "Sign in to directory..." command to sign in to additional tenants.`,
							),
						],
					});
					...
					if (tenantsWithSubs.length <= 1) {
						...
						const duplicates = tenantsWithSubs.filter(
							(tenant, index, self) =>
								self.findIndex(
						...
						const picks = tenantsWithSubs
							.map((tenant) => {
								...
								const isDuplicate = duplicates.some(
									(dup) => dup.tenantId === tenant.tenantId,
								);
								return <
									IAzureQuickPickItem<TenantIdDescription>
								>{
									label: isDuplicate
										? `${tenant.displayName} (${tenant.account.label})`
										: tenant.displayName,
				...
				serverQueue.push({
					...
					args: [
						localize(
							"usingTenant",
							`Using "${selectedTenant.displayName}" tenant.`,
						),
					],
				});
				...
				if (subscriptionsInSelectedTenant.length === 0) {
					...
					serverQueue.push({
						...
						args: [
							localize(
								"noSubscriptions",
								`Error: No subscriptions found for the selected account and tenant.`,
							),
						],
					});
					updateStatus("Disconnected");
				...
				const consoleUris: ConsoleUris = await connectTerminal(
					session.accessToken,
					consoleUri!,
					/* TODO: Separate Shell from OS */ osName === "Linux"
						? "bash"
						: "pwsh",
					initialSize,
					progressTask,
...
async function findUserSettings(
	...
	const userSettings: UserSettings | undefined =
		await getUserSettings(accessToken);
	// Valid settings will have either a storage profile (mounted) or a session type of 'Ephemeral'.
	if (
		userSettings &&
		(userSettings.storageProfile ||
			userSettings.sessionType === "Ephemeral")
	) {
		return userSettings;
	}
...
export async function provisionConsole(
	...
	let response = await createTerminal(
		accessToken,
		userSettings,
		osType,
		true,
	...
	for (
		...
		if (response.status < 200 || response.status > 299) {
			...
			if (
				...
				response.body &&
				body.error &&
				body.error.code === Errors.DeploymentOsTypeConflict
			) {
...
export async function connectTerminal(
	...
	for (let i = 0; i < 10; i++) {
		/* eslint-disable @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-assignment */
		const response = await initializeTerminal(
			accessToken,
			consoleUri,
			shellType,
			initialSize,
