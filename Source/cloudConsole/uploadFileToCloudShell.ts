...
import { createCloudConsole, OSes, OSName, shells } from "./cloudConsole";

async function cloudConsole(os: OSName) {
	const shell = createCloudConsole(
...
export async function uploadFileToCloudShell(
	...
	(async () => {
		if (!workspace.isTrusted) {
			throw new Error(
				localize(
					"uploadingRequiresTrustedWorkspace",
		...
		if (!shell) {
			const shellName = await window.showInformationMessage(
				localize(
					"uploadingRequiresOpenCloudConsole",
