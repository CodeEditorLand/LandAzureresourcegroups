...
} from "./api/ResourceProviderManagers";
...
export async function activate(
	...
	const refreshTenantTreeEmitter = new vscode.EventEmitter<
		void | ResourceGroupsItem | ResourceGroupsItem[] | null | undefined
	>();
	...
	ext.actions.refreshTenantTree = (data) =>
		refreshTenantTreeEmitter.fire(data);
		...
		async (activateContext: IActionContext) => {
			...
			context.subscriptions.push(
				vscode.window.registerTerminalProfileProvider(
					"azureResourceGroups.cloudShellBash",
					{
	...
	const azureResourceBranchDataProviderManager =
		new AzureResourceBranchDataProviderManager(
			new DefaultAzureResourceBranchDataProvider(),
			(type) =>
				void extensionManager.activateApplicationResourceBranchDataProvider(
					type,
				),
		);
	...
	const azureResourceProviderManager = new AzureResourceProviderManager(() =>
		extensionManager.activateApplicationResourceProviders(),
	);

	azureResourceProviderManager.addResourceProvider(
		new DefaultAzureResourceProvider(),
	);

	const workspaceResourceBranchDataProviderManager =
		new WorkspaceResourceBranchDataProviderManager(
			new WorkspaceDefaultBranchDataProvider(),
			(type) =>
				void extensionManager.activateWorkspaceResourceBranchDataProvider(
					type,
				),
		);
	const workspaceResourceProviderManager =
		new WorkspaceResourceProviderManager(() =>
			extensionManager.activateWorkspaceResourceProviders(),
		);

	const tenantResourceBranchDataProviderManager =
		new TenantResourceBranchDataProviderManager(
			new TenantDefaultBranchDataProvider(),
		);
	const tenantResourceProviderManager = new TenantResourceProviderManager(
		async () => {
			return undefined;
		},
	);
	...
	ext.v2.api = v2ApiFactory.createApi({
		extensionId: "ms-azuretools.vscode-azureresourcegroups",
	});
	...
	const getSubscriptions: (
		filter: boolean,
	) => Promise<AzureSubscription[]> = async (filter: boolean) => {
		return await (
			await getAzureSubscriptionProvider(azureResourceTreeDataProvider)
