...
export function survey({ globalState }: ExtensionContext): void {
	void callWithTelemetryAndErrorHandling(
		"azureResourceGroups.nps.survey",
		async (context: IActionContext) => {
			...
			const extensionVersion = (
				ext.context.extension.packageJSON as { version: string }
			).version;
			if (!isCandidate) {
			...
			const take = {
				title: localize(
					"azureResourceGroups.takeSurvey",
					"Take Survey",
				),
				run: async () => {
			...
			const remind = {
				title: localize(
					"azureResourceGroups.remindLater",
					"Remind Me Later",
				),
				run: async () => {
			...
			const never = {
				title: localize(
					"azureResourceGroups.neverAgain",
					"Don't Show Again",
				),
				isSecondary: true,
				run: async () => {
			...
			const button = await window.showInformationMessage(
				localize(
					"azureResourceGroups.surveyQuestion",
