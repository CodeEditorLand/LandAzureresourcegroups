...
export class AzExtWrapper {
	...
	public matchesResourceType(resource: AppResource): boolean {
		return this._resourceTypes.some(
			(rt) => rt === resource.azExtResourceType
		);
	}
	...
	public meetsMinVersion(): boolean {
		...
		return (
			this.getCodeExtension()?.packageJSON?.contributes?.[
				contributesKey
			] !== undefined
		);
	}
