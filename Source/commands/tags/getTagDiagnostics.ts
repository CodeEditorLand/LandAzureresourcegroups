...
class TagVisitor implements jsonc.JSONVisitor {
	 ...
	 */
	public onObjectBegin = (
		_offset: number,
		_length: number,
		startLine: number,
		startCharacter: number,
	) => {
		this._objectOpenBracketPositions.push(
	...
	public onArrayBegin = (
		...
		this._arrayOpenBracketPositions.push(
			new Position(startLine, startCharacter),
		);
	};
		...
		if (this._objectOpenBracketPositions.length === 1) {
			...
			if (property.length > max) {
				this.addError(
					...
					localize(
						"tagNameTooLong",
						"Tag name must be {0} characters or less.",
						max,
					),
				);
			}
		...
		if (this._objectOpenBracketPositions.length === 0) {
			...
			if (value.length > max) {
				this.addError(
					...
					localize(
						"tagValueTooLong",
						"Tag value must be {0} characters or less.",
						max,
					),
				);
			}
	...
	private addTagsTypeError(range: Range, actualType: string): void {
		this.addError(
			...
			localize(
				"tagsTypeError",
				'Tags must be an object of key/value pairs instead of "{0}".',
				actualType,
			),
		);
	}
	...
	private addTagValueTypeError(range: Range, actualType: string): void {
		this.addError(
			...
			localize(
				"tagTypeError",
				'Tag value must be of type "string" instead of "{0}".',
				actualType,
			),
		);
	}
