...
export class ResourceTags implements ITagsModel {
	constructor(private readonly resource: AzureResource) {
		this.displayType = resource.resourceGroup
			? "resource"
			: "resource group";
	}
	...
	async getTags(): Promise<Tags["tags"]> {
		return (
			(await callWithTelemetryAndErrorHandling(
				"getTags",
				async (context): Promise<Tags["tags"] | undefined> => {
					...
					if (this.resource.resourceGroup) {
						...
						const resources = await uiUtils.listAllIterator(
							client.resources.listByResourceGroup(
...
export class TagFileSystem extends AzExtTreeFileSystem<ITagsModel> {
	...
	public async writeFileImpl(
		...
		if (diagnostics.length > 0) {
			...
			context.telemetry.properties.diagnosticSources = diagnostics
				.map((d) => d.source)
				.filter(
					(value, index, array) =>
			...
			// remove example tag
			if (
				Object.keys(tags).includes(insertKeyHere) &&
	...
	private async getTagsFromNode(
		node: ITagsModel
	): Promise<Tags["tags"] | undefined> {
