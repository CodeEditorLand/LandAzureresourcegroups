...
export async function revealResource(
	...
	try {
		const item: ResourceGroupsItem | undefined = await (
		...
		if (item) {
			await ext.appResourceTreeView.reveal(
				...
				options ?? { expand: false, focus: true, select: true },
			);
		}
	} catch (error) {
		context.telemetry.properties.revealError = maskUserInfo(
...
function setTelemetryPropertiesForId(
	context: IActionContext,
	resourceId: string,
): void {
	...
	if (resourceKind === "resource") {
		context.telemetry.properties.resourceType =
			parsedAzureResourceId.provider?.replace(/\//g, "|");
	}
}

function parsePartialAzureResourceId(
	id: string,
): Partial<ParsedAzureResourceId> & Pick<ParsedAzureResourceId, "rawId"> {
...
function getResourceKindFromId(
	parsedId: Partial<ParsedAzureResourceId>,
