...
import {
	isResourceGroupGroupingItem,
	ResourceGroupGroupingItem,
} from "../../../tree/azure/grouping/ResourceGroupGroupingItem";
...
export async function deleteResourceGroupV2(
	context: IActionContext,
	primaryNode?: ResourceGroupGroupingItem,
	selectedNodes?: ResourceGroupGroupingItem[],
): Promise<void> {
	...
	const selectedResourceGroupNodes = getSelectedNodes(
		primaryNode,
		selectedNodes,
	);
	...
	if (selectedResourceGroupNodes) {
		...
		({ subscription, resourceGroupsToDelete } =
			await pickResourceGroups(context));
	}
...
async function pickResourceGroups(context: IActionContext) {
	...
	const pickedResourceGroups = await context.ui.showQuickPick<
		IAzureQuickPickItem<ResourceGroup>
	>(picks, {
		canPickMany: true,
		placeHolder: localize(
			"selectResourceGroupToDelete",
	...
	return {
		subscription,
		resourceGroupsToDelete: pickedResourceGroups.map((pick) =>
			createResourceGroup(subscription, pick.data),
		),
	};
}

async function deleteResourceGroups(
	context: IActionContext,
	...
	for await (const rg of resourceGroups) {
		...
		if (deleteConfirmation === "ClickButton") {
			const areYouSureDelete: string = localize(
				"areYouSureDelete",
				'Are you sure you want to delete resource group "{0}"? There are {1} resources in this resource group that will be deleted.',
				rg.name,
				numOfResources,
			);
			...
			const enterToDelete: string = localize(
				"enterToDelete",
				'Enter "{0}" to delete this resource group. There are {1} resources in this resource group that will be deleted.',
				rg.name,
				numOfResources,
			);
		...
		void ext.azureTreeState.runWithTemporaryDescription(
			rg.id,
			localize("deleting", "Deleting..."),
			async () => {
