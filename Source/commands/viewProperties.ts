...
export async function viewProperties(
	...
	if (!node) {
		node = await azureResourceExperience<ResourceGroupsItem>(
	...
	if (!hasViewProperties(node)) {
		throw new Error(
			localize(
				"commands.viewProperties.noProperties",
				"The selected resource has no properties to view."
			)
		);
	}
...
export function hasViewProperties(
	node: unknown
): node is { viewProperties: ViewPropertiesModel } {
	return !!(node as { viewProperties: ViewPropertiesModel })?.viewProperties;
}

function isAsyncViewPropertiesModel(
	viewProperties: ViewPropertiesModel
): viewProperties is ViewPropertiesModel & ViewPropertiesModelAsync {
