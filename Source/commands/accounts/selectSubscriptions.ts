...
export async function selectSubscriptions(
	...
	if (await provider.isSignedIn()) {
		...
		const subscriptionQuickPickItems: () => Promise<
		...
		> = async () => {
			...
			if (options?.account || options?.tenantId) {
				if (options?.account) {
					subscriptions = subscriptions.filter(
						(subscription) =>
							subscription.account.id === options.account?.id,
					);
				}
				if (options?.tenantId) {
					subscriptions = subscriptions.filter(
			...
			subscriptionsShownInPicker = subscriptions.map(
				...
				isPickSelected: (pick) => {
					return (
						...
						selectedSubscriptionIds.includes(
							(pick as IAzureQuickPickItem<AzureSubscription>)
								.data.subscriptionId,
						)
					);
				},
		...
		if (picks) {
			...
			const previouslySelectedSubscriptionsSettingValue = new Set(
				selectedSubscriptionsWithFullId,
			);

			// remove any that were shown in the picker
			subscriptionsShownInPicker.forEach((pick) =>
				previouslySelectedSubscriptionsSettingValue.delete(pick),
			);

			// add any that were selected in the picker
			picks.forEach((pick) =>
				previouslySelectedSubscriptionsSettingValue.add(
					`${pick.data.tenantId}/${pick.data.subscriptionId}`,
				),
			);

			// update the setting
			await setSelectedTenantAndSubscriptionIds(
				Array.from(previouslySelectedSubscriptionsSettingValue),
			);
		}
...
async function setSelectedTenantAndSubscriptionIds(
	tenantAndSubscriptionIds: string[],
): Promise<void> {
	await settingUtils.updateGlobalSetting(
		"selectedSubscriptions",
		tenantAndSubscriptionIds,
	);
}

// This function is also used to filter subscription tree items in AzureResourceTreeDataProvider
export function getTenantFilteredSubscriptions(
...
export function getDuplicateSubscriptions(
	...
	const lookup = subscriptions.reduce(
		(accumulator, sub) => {
			accumulator[sub.subscriptionId] =
				++accumulator[sub.subscriptionId] || 0;
			return accumulator;
		},
