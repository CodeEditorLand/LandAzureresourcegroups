...
export async function configureSovereignCloud(
	context: ConfigureSovereignCloudContext,
): Promise<void> {
	...
	const title: string = localize(
		"selectSovereignCloud",
		"Select Sovereign Cloud",
	);

	const wizard: AzureWizard<ConfigureSovereignCloudContext> = new AzureWizard(
		wizardContext,
		{
			title,
