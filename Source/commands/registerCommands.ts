...
import {
	...
	selectSubscriptions,
} from "./accounts/selectSubscriptions";
...
export function registerCommands(): void {
	registerCommand(
		...
		uploadFileToCloudShell,
	);

	registerCommand(
		"azureResourceGroups.maintainCloudShellConnection",
		maintainCloudShellConnection,
	);
	...
	registerCommand(
		...
		async (_context, node: TenantTreeItem) => {
			await (
				await ext.subscriptionProviderFactory()
			).signIn(node.tenantId, node.account);
			ext.actions.refreshTenantTree(node);
		},
	);
	...
	registerCommand("azureTenantsView.addAccount", (context: IActionContext) =>
		logIn(context),
	);
	registerCommand(
	...
	registerCommand("ms-azuretools.openWalkthrough", () =>
		commands.executeCommand(
			...
			`ms-azuretools.vscode-azureresourcegroups#azure-get-started`,
		),
	);
	...
	registerCommand(
		...
		async (context: IActionContext, node: AzExtTreeItem) =>
			await ext.workspaceTree.loadMore(node, context),
	);

	registerCommand(
		"azureTenantsView.configureSovereignCloud",
		configureSovereignCloud,
	);
}

async function handleAzExtTreeItemRefresh(
