...
export async function maintainCloudShellConnection(
	_context: IActionContext,
	options: MaintainCloudShellConnectionOptions,
) {
	try {
...
async function cloudShellKeepAlive(consoleUri: string) {
	...
	if (response.ok) {
		ext.outputChannel.debug(
			"Successfully called Cloud Shell keepAlive endpoint.",
		);
	} else {
...
async function cloudShellSize(
	...
	const sizeResponse = await fetch(
		`${consoleUri}/terminals/${terminalId}/size?cols=68&rows=15&api-version=2019-01-01`,
		{
			method: "POST",
			headers: {
				"Authorization": `Bearer ${accessToken}`,
	...
	if (sizeResponse.ok) {
		ext.outputChannel.debug(
			"Successfully called Cloud Shell size endpoint.",
		);
	} else {
...
async function recordTelemetry(options: { templateUrl?: string }) {
	await callWithTelemetryAndErrorHandling(
		"vscode-dev-azure.cloudShellConnection",
		async (context) => {
