...
function getPicks(
	createResourceCommands: ContributedCreateResourceCommand[],
): IAzureQuickPickItem<ContributedCreateResourceCommand>[] {
	return createResourceCommands
		...
		.map((command) => ({
			...
			iconPath:
				command.type &&
				Object.values(AzExtResourceType).includes(command.type)
					? getIconPath(command.type)
					: undefined,
		}));
}

function getContributedCreateResourceCommands(): ContributedCreateResourceCommand[] {
	const getCommandsForExtension = (
		extension: Extension<unknown>,
	): ContributedCreateResourceCommand[] | undefined =>
		getResourceContributions(extension)?.commands?.map((command) => ({
