...
export class ActivityLogTreeItem
	...
	implements Disposable
{
	...
	public dispose(): void {
		Object.values(this.activityTreeItems).forEach(
	...
	public async addActivity(activity: Activity): Promise<void> {
		await callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				this.activityTreeItems[activity.id] = new ActivityTreeItem(
					this,
					activity
				);
				if (
	...
	public async clearActivities(context: IActionContext): Promise<void> {
		Object.entries(this.activityTreeItems).forEach(
	...
	public async loadMoreChildrenImpl(
		...
		return Object.values(this.activityTreeItems).filter(
			(activity) => !!activity.status
		);
	}

	public compareChildrenImpl(
		item1: ActivityTreeItem,
		item2: ActivityTreeItem
	): number {
