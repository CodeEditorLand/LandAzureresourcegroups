...
function handleError(e: unknown, functionName: string): never {
	...
	if (e instanceof Error) {
		...
		e.message = `Internal error: '${functionName}' threw exception ${
...
type ObjectWithFunctions = Record<
	symbol,
	(...args: unknown[]) => unknown | Promise<unknown>
>;
...
> = (
	...args: Parameters<T>
) => ReturnType<T> extends Promise<infer TPromise>
	? Promise<TPromise>
 ...
 * ```
 */
export function wrapFunctionsInTelemetry<
	TFunctions extends ObjectWithFunctions,
>(
	functions: TFunctions,
	options?: WrapFunctionsInTelemetryOptions
): AsyncFunctions<TFunctions> {
	const wrappedFunctions: Record<string, (...args: unknown[]) => unknown> =
		{};

	Object.entries(
		functions as Record<string, (...args: unknown[]) => unknown>
	).forEach(([functionName, func]) => {
		wrappedFunctions[functionName] = (
 ...
 * ```
 */
export function wrapFunctionsInTelemetrySync<
	TFunctions extends ObjectWithFunctions,
>(
	functions: TFunctions,
	options?: WrapFunctionsInTelemetryOptions
