...
export function createGroupConfigFromResource(
	...
	const groupConfig: GroupingConfig = {
		resourceGroup: {
			...
			id: id
				.substring(0, id.indexOf("/providers"))
				.toLowerCase()
				.replace("/resourcegroups", "/resourceGroups"),
			contextValuesToAdd: ["azureResourceGroup"],
		},
		resourceType: {
			label: resource.azExtResourceType
				? (azExtDisplayInfo[resource.azExtResourceType ?? ""]
						?.displayName ?? resource.azExtResourceType)
				: unknown,
			...
			contextValuesToAdd: [
				...
				...(resource.azExtResourceType
					? [
							resource.azExtResourceType,
							legacyTypeMap[resource.azExtResourceType] ?? "",
						]
					: []),
			],
		},
...
export function createAzureExtensionsGroupConfig(
	extensions: IAzExtMetadata[],
	...
	for (const azExt of extensions) {
		for (const azExtResourceType of azExt.resourceTypes) {
			azExtGroupConfigs.push({
				label:
					azExtDisplayInfo[azExtResourceType]?.displayName ??
					azExtResourceType,
				...
				contextValuesToAdd: [
					"azureResourceTypeGroup",
					azExtResourceType,
					legacyTypeMap[azExtResourceType] ?? "",
				],
			});
		}
	}
...
export function getName(
	azExtResourceType?: AzExtResourceType,
): string | undefined {
	return azExtResourceType
		? azExtDisplayInfo[azExtResourceType]?.displayName
		: undefined;
}
...
const azExtDisplayInfo: Partial<
	Record<AzExtResourceType, AzExtResourceTypeDisplayInfo>
> = {
	ApplicationInsights: {
		displayName: localize("insightsComponents", "Application Insights"),
	},
	...
	AppServicePlans: {
		displayName: localize("serverFarms", "App Service plans"),
	},
	...
	AvailabilitySets: {
		displayName: localize("availabilitySets", "Availability sets"),
	},
	...
	ContainerAppsEnvironment: {
		displayName: localize("containerAppsEnv", "Container Apps"),
	},
	ContainerRegistry: {
		displayName: localize("containerRegistry", "Container registry"),
	},
	...
	NetworkInterfaces: {
		displayName: localize("networkInterfaces", "Network interfaces"),
	},
	...
	NetworkWatchers: {
		displayName: localize("networkWatchers", "Network watchers"),
	},
	...
	PublicIpAddresses: {
		displayName: localize("publicIpAddresses", "Public IP addresses"),
	},
	...
	StorageAccounts: {
		displayName: localize("storageAccounts", "Storage accounts"),
	},
	VirtualMachines: {
		displayName: localize("virtualMachines", "Virtual machines"),
	},
	...
	VirtualNetworks: {
		displayName: localize("virtualNetworks", "Virtual networks"),
	},
