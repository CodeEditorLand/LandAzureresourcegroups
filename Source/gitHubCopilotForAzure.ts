 ...
 */
export function gitHubCopilotForAzureToast({
	globalState,
	...
	void callWithTelemetryAndErrorHandling(
		...
		async (context: IActionContext) => {
			...
			const arePrecursorExtensionsInstalled: boolean =
				isExtensionInstalled(ghcpExtensionId) &&
				isExtensionInstalled(ghcpChatExtensionId);

			if (
				!arePrecursorExtensionsInstalled ||
				isExtensionInstalled(ghcpfaExtensionId)
			) {
				return;
			}

			const dontShow: boolean = globalState.get<boolean>(
				dontShowKey,
				false,
			);
			...
			const message: string = localize(
				"ghcpToastMessage",
				"Get help with Azure questions and tasks in Copilot Chat by installing the GitHub Copilot for Azure extension.",
			);
...
function isExtensionInstalled(extensionId: string): boolean {
	const extension: Extension<unknown> | undefined =
