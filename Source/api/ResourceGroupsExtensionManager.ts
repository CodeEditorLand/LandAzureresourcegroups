...
export class ResourceGroupsExtensionManager {
	async activateApplicationResourceBranchDataProvider(
		type: AzExtResourceType
	): Promise<void> {
		const extensionAndContributions = getInactiveExtensions()
			.map((extension) => ({
				extension,
				contributions:
					getResourceContributions(extension)?.azure?.branches?.map(
						(resource) => resource.type
					) ?? [],
			}))
			.find(
				(extensionAndContributions) =>
					extensionAndContributions.contributions.find(
	...
	async activateApplicationResourceProviders(): Promise<void> {
		const inactiveResourceContributors = getInactiveExtensions().filter(
			(extension) => getResourceContributions(extension)?.azure?.resources
		);

		await Promise.all(
			inactiveResourceContributors.map((extension) =>
				extension.activate()
			)
		);
	}
	...
	async activateWorkspaceResourceProviders(): Promise<void> {
		const inactiveResourceContributors = getInactiveExtensions().filter(
			(extension) =>
				getResourceContributions(extension)?.workspace?.resources
		);
	...
	async activateWorkspaceResourceBranchDataProvider(
		type: string
	): Promise<void> {
		const extensionAndContributions = getInactiveExtensions()
			.map((extension) => ({
