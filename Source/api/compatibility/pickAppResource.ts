...
export function createCompatibilityPickAppResource() {
	return async function pickAppResource<T extends AzExtTreeItem>(
		context: ITreeItemPickerContext,
		options?: PickAppResourceOptions,
	): Promise<T> {
		const result = await PickTreeItemWithCompatibility.resource<T>(
			context,
			ext.v2.api.resources.azureResourceTreeDataProvider,
			{
				...
				// @ts-ignore
				resourceTypes: convertAppResourceFilterToAzExtResourceType(
					options?.filter,
				),
				childItemFilter:
					convertExpectedChildContextValueToContextValueFilter(
...
function convertExpectedChildContextValueToContextValueFilter(
	expectedChildContextValue?: PickAppResourceOptions["expectedChildContextValue"],
): ContextValueFilter | undefined {
	return expectedChildContextValue
		? { include: expectedChildContextValue }
		: undefined;
}

function convertAppResourceFilterToAzExtResourceType(
	filter?: PickAppResourceOptions["filter"],
