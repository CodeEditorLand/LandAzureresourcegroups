...
export const applicationResourceResolvers: Partial<
	Record<AzExtResourceType, AppResourceResolver>
> = {};

export function registerApplicationResourceResolver(
	...
	return callWithTelemetryAndErrorHandlingSync(
		...
		() => {
			if (applicationResourceResolvers[type]) {
				throw new Error(
					`Application resource resolver with id '${type}' has already been registered.`,
				);
			}
