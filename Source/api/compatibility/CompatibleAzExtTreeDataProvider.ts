...
export class CompatibleAzExtTreeDataProvider extends IntermediateCompatibleAzExtTreeDataProvider {
	public constructor(private readonly tdp: ResourceTreeDataProviderBase) {
		super(
			new CompatibleAzureAccountTreeItem(),
			undefined as unknown as string,
		);
	}

	public override getParent(
	...
	public override async findTreeItem<T>(
		fullId: string,
	): Promise<T | undefined> {
	...
	public override showTreeItemPicker<T extends AzExtTreeItem>(
	...
	public override async showTreeItemPicker<T extends AzExtTreeItem>(
		...
		if (expectedContextValues === SubscriptionTreeItemBase.contextValue) {
			const subscriptionContext =
				await PickTreeItemWithCompatibility.subscription(
					context,
					this.tdp,
				);
		...
		return PickTreeItemWithCompatibility.showTreeItemPicker<T>(
			context,
			this.tdp,
	...
	public override trackTreeItemCollapsibleState(
		_treeView: TreeView<AzExtTreeItem>,
	): Disposable {
		throw new ShouldNeverBeCalledError(
			"trackTreeItemCollapsibleState method",
		);
	}
	...
	public get onDidExpandOrRefreshExpandedTreeItem(): Event<AzExtTreeItem> {
		throw new ShouldNeverBeCalledError(
			"onDidExpandOrRefreshExpandedTreeItem accessor",
		);
	}
