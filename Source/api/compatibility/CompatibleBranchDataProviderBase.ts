...
export abstract class CompatibleBranchDataProviderBase<
		TResource extends ResourceBase,
		TModel extends AzExtTreeItem & ResourceModelBase,
	>
	extends AzExtTreeDataProvider
	implements BranchDataProvider<TResource, TModel>, vscode.Disposable
{
	protected readonly overrideOnDidChangeTreeDataEmitter =
		new vscode.EventEmitter<TModel | undefined>();
	...
	public override get onDidChangeTreeData(): vscode.Event<
		TModel | undefined
	> {
		return this.overrideOnDidChangeTreeDataEmitter.event;
	}

	public override set onDidChangeTreeData(
		_: vscode.Event<TModel | undefined>
	) {
		// Do nothing
	}
	...
	public override async getChildren(
		treeItem: TModel & AzExtParentTreeItem
	): Promise<TModel[]> {
	...
	public override async showTreeItemPicker<T extends AzExtTreeItem>(
		...
		_startingTreeItem?: AzExtTreeItem
	): Promise<T> {
		throw new Error(
			"Use the Resources extension API to do showTreeItemPicker"
		);
	}

	public override async findTreeItem<T extends AzExtTreeItem>(
		_fullId: string,
		_context: IFindTreeItemContext
	): Promise<T | undefined> {
	...
	private override get _rootTreeItem(): AzExtParentTreeItem {
		throw new Error(
			"The root tree item should not be accessed in a BranchDataProvider"
		);
	}
