...
export class CompatibleResolvedApplicationResourceTreeItem extends AzExtParentTreeItem {
	...
	public get collapsibleState(): TreeItemCollapsibleState | undefined {
		return (this.resolveResult?.initialCollapsibleState ??
			!!this.resolveResult?.loadMoreChildrenImpl)
	...
	public static Create(
		...
		resolveResult: ResolvedAppResourceBase,
		subscription: ISubscriptionContext,
		treeDataProvider: AzExtTreeDataProvider,
		azureResource: AzureResource,
	): CompatibleResolvedApplicationResourceTreeItem {
		const resolvable: CompatibleResolvedApplicationResourceTreeItem =
			new CompatibleResolvedApplicationResourceTreeItem(
				resource,
				resolveResult,
				subscription,
				treeDataProvider,
	...
	private constructor(
		resource: AzureResource,
		resolved: ResolvedAppResourceBase,
		__subscription: ISubscriptionContext,
		__treeDataProvider: AzExtTreeDataProvider,
		azureResource: AzureResource,
	) {
		super((<Partial<AzExtParentTreeItem>>{
...
export function createResolvableProxy<T extends AzExtParentTreeItem>(
	...
	const providerHandler: ProxyHandler<Resolvable<T>> = {
		...
		set: (target: Resolvable<T>, name: string, value: unknown): boolean => {
			if (
				resolvable.resolveResult &&
				Object.getOwnPropertyDescriptor(resolvable.resolveResult, name)
					?.writable
			) {
		...
		getPrototypeOf: (
			target: Resolvable<T>,
		): AzExtParentTreeItem | AzExtTreeItem => {
			if (resolvable?.resolveResult) {
				return resolvable.resolveResult.loadMoreChildrenImpl
