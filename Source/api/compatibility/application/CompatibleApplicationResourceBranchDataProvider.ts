 ...
 */
export class CompatibleApplicationResourceBranchDataProvider<
	TResource extends AzureResource,
	TModel extends AzExtTreeItem & ResourceModelBase,
> extends CompatibleBranchDataProviderBase<TResource, TModel> {
	public constructor(
		private readonly resolver: AppResourceResolver,
	...
	public async getResourceItem(element: TResource): Promise<TModel> {
		...
		const resolved = await this.resolver.resolveResource(
			subscriptionContext,
			oldAppResource,
		);
		if (!resolved) {
