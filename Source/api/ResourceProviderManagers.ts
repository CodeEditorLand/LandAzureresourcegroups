...
class ResourceProviderManager<
	...
	TResourceProvider extends ResourceProvider<TResourceSource, TResource>,
> extends vscode.Disposable {
	...
	public readonly onDidChangeResourceChange: vscode.Event<
		TResource | undefined
	>;
	...
	constructor(private readonly extensionActivator: () => Promise<void>) {
		...
		this.onDidChangeResourceChange =
			this.onDidChangeResourceEventEmitter.event;
	}

	addResourceProvider(resourceProvider: TResourceProvider): void {
		this.providers.set(resourceProvider, {
			listener: resourceProvider.onDidChangeResource?.((resource) =>
				this.onDidChangeResourceEventEmitter.fire(resource),
			),
		});
	...
	async getResources(source: TResourceSource): Promise<TResource[]> {
		...
		const resources = await Promise.all(
			resourceProviders.map((resourceProvider) =>
				resourceProvider.getResources(source),
			),
		);
...
export class AzureResourceProviderManager extends ResourceProviderManager<
	AzureSubscription,
	AzureResource,
	AzureResourceProvider
> {}

export class WorkspaceResourceProviderManager extends ResourceProviderManager<
	void,
	WorkspaceResource,
	WorkspaceResourceProvider
> {}

export class TenantResourceProviderManager extends ResourceProviderManager<
	void,
	TenantResource,
