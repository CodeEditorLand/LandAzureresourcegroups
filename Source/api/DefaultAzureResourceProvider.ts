...
export class DefaultAzureResourceProvider implements AzureResourceProvider {
	private readonly onDidChangeResourceEmitter = new vscode.EventEmitter<
	...
	getResources(
		...
		return callWithTelemetryAndErrorHandling(
			...
			async (context: IActionContext) => {
				const azureResources = await this.listResources(
					context,
					subscription
				);
	...
	private async listResources(
		...
		const allResources = await getAzureResourcesService().listResources(
			context,
			subscription
		);
		...
		return allResourcesDeduped.map((resource) =>
			createAzureResource(subscription, resource)
		);
	}

	private async listResourceGroups(
		context: IActionContext,
		subscription: AzureSubscription
	): Promise<AzureResource[]> {
		const allResourceGroups: ResourceGroup[] =
			await getAzureResourcesService().listResourceGroups(
				context,
				subscription
			);
