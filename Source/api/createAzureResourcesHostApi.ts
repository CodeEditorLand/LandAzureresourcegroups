...
import {
	...
	AzureResourcesHostApiInternal,
} from "../hostapi.v2.internal";
...
export function createAzureResourcesHostApi(
	...
	return {
		...
		registerAzureResourceBranchDataProvider: <T extends ResourceModelBase>(
		...
		registerWorkspaceResourceBranchDataProvider: <
			T extends ResourceModelBase,
		>(
			type: string,
			provider: BranchDataProvider<WorkspaceResource, T>,
		) => {
			workspaceResourceBranchDataProviderManager.addProvider(
				type,
				provider,
			);
