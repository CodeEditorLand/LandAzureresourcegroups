...
async function mockResourcesAndGetSubscriptionItems() {
	...
	const tdp = api()
		.azureResourceTreeDataProvider as AzureResourceTreeDataProvider;
...
suite("SubscriptionItem tests", async () => {
	test("Tree should show a subscription item for each Azure subscription", async () => {
		const { mockResources, subscriptionItems } =
			await mockResourcesAndGetSubscriptionItems();
		mockResources.subscriptions.forEach((subscription) => {
			assert.ok(
				subscriptionItems?.find((node) => node.id === subscription.id)
			);
		});

		assert.strictEqual(
			subscriptionItems.length,
			mockResources.subscriptions.length,
			`There should be ${mockResources.subscriptions.length} subscription nodes. Found ${subscriptionItems.length}.`
		);
	});
	...
	test('SubscriptionItem.id should be in the "/subscriptions/<subscription id>" format', async () => {
		...
		subscriptionItems.forEach((subscriptionItem) => {
			assert.match(
				subscriptionItem.id,
				/^\/subscriptions\/([0-9a-fA-F]{8}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{4}\b-[0-9a-fA-F]{12})/i
			);
		});
	});
