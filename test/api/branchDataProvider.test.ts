...
const getWorkspaceResourceProviderStub: (
	onCalled?: () => void,
	resources?: WorkspaceResource[]
) => WorkspaceResourceProvider = (onCalled, resources) => {
	return {
		getResources: async () => {
			onCalled?.();
			return resources ?? [];
...
suite("Branch data provider tests", async () => {
	test("Provided workspace tree items are displayed", async () => {
		...
		const provider = getWorkspaceResourceProviderStub(undefined, [
			workspaceResource,
		]);
		api().registerWorkspaceResourceProvider(provider);
		api().registerWorkspaceResourceBranchDataProvider(
		...
		const children =
			(await api().workspaceResourceTreeDataProvider.getChildren()) as any[];
	...
	test("BranchDataProvider.getTreeItem should be called for resource items", async () => {
		...
		const rootChildren =
			(await api().workspaceResourceTreeDataProvider.getChildren()) as any[];
		const testChild = rootChildren.find(
			(c) => c.id === workspaceResource.id
		);
		branchDataProvider.assertGetTreeItemCalledAsync(async () => {
			await api().workspaceResourceTreeDataProvider.getTreeItem(
				testChild
			);
		});
	});
	...
	test("BranchDataProvider.getTreeItem should be called for resource item children", async () => {
		...
		api().registerWorkspaceResourceProvider(
			getWorkspaceResourceProviderStub(undefined, [workspaceResource])
		);

		const branchDataProvider = new TestBranchDataProvider();
		branchDataProvider.registerChildren(workspaceResource, [
			workspaceResourceChild,
		]);

		api().registerWorkspaceResourceBranchDataProvider(
			workspaceResource.resourceType,
			branchDataProvider
		);

		const workspaceResourceNodes =
			(await api().workspaceResourceTreeDataProvider.getChildren()) as WorkspaceResource[];
		const workspaceResourceNode = workspaceResourceNodes.find(
			(c) => c.id === workspaceResource.id
		);

		assert.ok(
			workspaceResourceNode,
			`No workspace resource node found with id: ${
				workspaceResource.id
			}. Workspace resource nodes: ${workspaceResourceNodes
				.map((item) => item.id)
				.join(", ")}`
		);

		const workspaceResourceChildNodes =
			(await api().workspaceResourceTreeDataProvider.getChildren(
				workspaceResourceNode
		...
		assert.strictEqual(
			workspaceResourceChildNode.id,
			workspaceResourceChild.id
		);

		branchDataProvider.assertGetTreeItemCalledAsync(async () => {
			await api().workspaceResourceTreeDataProvider.getTreeItem(
				workspaceResourceChildNode
			);
		});
	});
	...
	test("Firing BranchDataProvider.onDidChangeTreeData event with a branch item should fire a TreeDataProvider.onDidChangeTreeData evente with the corresponding wrapper item.", async () => {
		...
		const waitForOnDidChangeTreeDataToFire = waitForEventToFire(
			tdp.onDidChangeTreeData!
		);
	...
	test("Firing BranchDataProvider.onDidChangeTreeData event with a wrapper item should result in TreeDataProvider.onDidChangeTreeData being fired with an empty array.", async () => {
		...
		branchDataProvider.onDidChangeTreeDataEmitter.fire(
			testChild as unknown as WorkspaceResource
		);
...
function setupTestBranchDataProvider() {
	...
	const tdp = api()
		.workspaceResourceTreeDataProvider as TreeDataProvider<unknown>;
	return { branchDataProvider, tdp, workspaceResource };
}

async function getGrandchild() {
	const { branchDataProvider, workspaceResource } =
		setupTestBranchDataProvider();
	const tdp = api()
		.workspaceResourceTreeDataProvider as TreeDataProvider<unknown>;
	...
	assert.ok(
		testChild,
		`No test child found with id: ${
			workspaceResource.id
		}. Test children: ${rootChildren.map((item) => item.id).join(", ")}`
	);
