...
suite("Azure Resource Branch Data Provider tests", async () => {
	test("Registered Azure resource branch data provider is used", async () => {
		...
		const azureResourceBranchDataProvider: BranchDataProvider<
			AzureResource,
			ResourceModelBase
		> = {
		...
		api().registerAzureResourceBranchDataProvider(
		...
		const functionGroup = groups!.find(
			(g) => g.label?.toString().includes("Func")
		);
	...
	test("Tree should be resilliant to errors thrown in BranchDataProvider.getResourceItem", async () => {
		...
		const children = (await tdp.getChildren(
			functionGroup
		)) as BranchDataItemWrapper[];

		const childTreeItems: TreeItem[] = await Promise.all(
			children.map((child) => tdp.getTreeItem(child))
		);

		assert.ok(children);

		const invalidTreeItems = childTreeItems.filter(
			(child) => child.contextValue?.includes("invalid")
		);
		assert.strictEqual(
			invalidTreeItems.length,
			1,
			`There should be 1 invalid tree item: ${invalidTreeItems
				.map((i) => i.label)
				.join(", ")}`
		);
	});
	...
	test("Should show a custom error message when BranchDataProvider.getResourceItem returns nullish value", async () => {
		...
		assert.doesNotMatch(
			invalidTreeItems[0].label!.toString(),
			/Cannot read properties of undefined/,
			`Error should not be a "Cannot read properties of undefined error`
		);
	});
});

export interface AzureResourceTreeDataProvider
	extends TreeDataProvider<ResourceGroupsItem> {
	getChildren(): Promise<SubscriptionItem[]>;
	getChildren(element: SubscriptionItem): Promise<GroupingItem[]>;
	getChildren(
		element: GroupingItem
	): Promise<AzureResourceItem<AzureResource>[]>;
	getChildren(
		element: AzureResourceItem<AzureResource>
	): Promise<BranchDataItemWrapper[]>;
}
