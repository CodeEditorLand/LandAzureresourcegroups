...
class MockSubscription {
	...
	get resources(): MockResource[] {
		return this.resourceGroups.reduce(
			(acc, rg) => acc.concat(rg.resources),
			[] as MockResource[]
		);
	}
}
...
function addResourceGroup(
	...
	resourceGroupOptions: { name: string; location?: string }
): MockResourceGroup {
	const mockResourceGroup = new MockResourceGroup(
		subscription.id,
		resourceGroupOptions.name,
		resourceGroupOptions.location ?? "westus2"
	);
...
function addResource(
	...
	resourceOptions: { name: string; type: string; kind: string }
): MockResource {
	const resource = new MockResource(
		resourceGroup.id,
		resourceOptions.name,
		resourceOptions.type,
...
function addFunctionApp(
	resoruceGroup: MockResourceGroup,
	name: string
): MockResource {
...
export const createMockSubscriptionWithFunctions = (): BasicMockResources => {
	...
	ext.testing.overrideAzureServiceFactory =
		createTestAzureResourcesServiceFactory(mockResources);
	const mockAzureSubscriptionProvider = new MockAzureSubscriptionProvider(
		mockResources
	);
	ext.testing.overrideAzureSubscriptionProvider = () =>
		mockAzureSubscriptionProvider;
	return mockResources;
};

export function createTestAzureResourcesServiceFactory(
	...
	return () => {
		return {
			async listResources(
				...
				if (
					mockResources.subscriptionsMap.has(
						subscription.subscriptionId
					)
				) {
					return mockResources.subscriptionsMap.get(
						subscription.subscriptionId
					)!.resources;
				}
				throw new Error(
