...
class PortalUrlBranchDataProvider
...
suite("AzureResourceModel.portalUrl tests", async () => {
	test(`TreeItem.contextValue should include "${BranchDataItemWrapper.hasPortalUrlContextValue}" if AzureResourceModel.portalUrl is defined`, async () => {
		...
		api().registerAzureResourceBranchDataProvider(
			...
			new PortalUrlBranchDataProvider(mockResources.functionApp1.id)
		);
		...
		const grandchildrenTreeItemsWithPortalUrl = grandChildTreeItems.filter(
			(treeItem) =>
				treeItem.contextValue?.includes(
					BranchDataItemWrapper.hasPortalUrlContextValue
				)
		);
		assert.strictEqual(
			grandchildrenTreeItemsWithPortalUrl.length,
			1,
			`There should be 1 tree item with "${BranchDataItemWrapper.hasPortalUrlContextValue}" context value`
		);
	});
});
