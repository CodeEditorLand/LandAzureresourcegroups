...
class ViewPropertiesBranchDataProvider
	implements AzureResourceBranchDataProvider<TestAzureResourceModel>
{
...
suite("AzureResourceModel.viewProperties tests", async () => {
	test(`TreeItem.contextValue should only include "${BranchDataItemWrapper.hasPropertiesContextValue}" if AzureResourceModel.viewProperties is defined`, async () => {
		...
		api().registerAzureResourceBranchDataProvider(
			...
			new ViewPropertiesBranchDataProvider(mockResources.functionApp1.id)
		);
		...
		const grandchildren = await Promise.all(
			children.map(
				(child) =>
					tdp.getChildren(child) as Promise<BranchDataItemWrapper[]>
			)
		);
		...
		const grandchildrenTreeItemsWithPortalUrl = grandChildTreeItems.filter(
			(treeItem) =>
				treeItem.contextValue?.includes(
					BranchDataItemWrapper.hasPropertiesContextValue
				)
		);
		assert.strictEqual(
			grandchildrenTreeItemsWithPortalUrl.length,
			1,
			`There should be 1 tree item with "${BranchDataItemWrapper.hasPropertiesContextValue}" context value`
		);
	});
	...
	test(`BrachDataItemWrapper should have viewPropertiesModel`, async () => {
		...
		const functionApp1Item = children.find(
			(child) => child.id === mockResources.functionApp1.id
		);
