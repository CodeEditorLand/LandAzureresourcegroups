...
import {
	...
	ext,
	GroupingItem,
	isLocationGroupingItem,
	isResourceGroupGroupingItem,
	isResourceTypeGroupingItem,
	LocationGroupingItem,
	ResourceGroupGroupingItem,
	ResourceTypeGroupingItem,
...
suite("Azure resource grouping tests", async () => {
	test("Group by resource type", async () => {
		...
		const groups = (await tdp.getChildren(
			subscriptions![0]
		)) as GroupingItem[];
		assert.ok(isResourceTypeGroupingItem(groups[0]));
		const functionGroup = groups.find(
			(group) =>
				(group as ResourceTypeGroupingItem).resourceType ===
	...
	test("Group by resource group", async () => {
		...
		const resourceGroup = groups.find(
			(group) =>
				(group as ResourceGroupGroupingItem).resourceGroup.id ===
				mocks.sub1.rg1.id
		);
	...
	test("Group by location", async () => {
		...
		const locationGroup = groups.find(
			(group) =>
				(group as LocationGroupingItem).location ===
				mocks.sub1.rg1.location
		);
