...
suite("getTagDiagnostics", () => {
	...
	const testCases: ITestCase[] = [
		...
		//#region Invalid tags type
		{
			name: "numberTags",
			text: 3,
			diagnostics: [{ range: [0, 0, 0, 1], error: invalidTagsType }],
		},
		{
			name: "booleanTags",
			text: true,
			diagnostics: [{ range: [0, 0, 0, 4], error: invalidTagsType }],
		},
		{
			name: "arrayTags",
			text: [],
			diagnostics: [{ range: [0, 0, 0, 2], error: invalidTagsType }],
		},
		{
			name: "nestedArrayTags",
			text: [[]],
			diagnostics: [{ range: [0, 0, 2, 1], error: invalidTagsType }],
		},
		//#endregion
		//#region Invalid prop type
		{
			name: "numberProp",
			text: { test: 3 },
			diagnostics: [{ range: [1, 12, 1, 13], error: invalidTagType }],
		},
		{
			name: "booleanProp",
			text: { test: true },
			diagnostics: [{ range: [1, 12, 1, 16], error: invalidTagType }],
		},
		{
			name: "arrayProp",
			text: { test: [] },
			diagnostics: [{ range: [1, 12, 1, 14], error: invalidTagType }],
		},
		{
			name: "nestedArrayProp",
			text: { test: [[]] },
			diagnostics: [{ range: [1, 12, 3, 5], error: invalidTagType }],
		},
		{
			name: "objectProp",
			text: { test: {} },
		...
		//#endregion
		{
			...
			text: { "tes%t": "test" },
			diagnostics: [{ range: [1, 4, 1, 11], error: invalidCharsKey }],
		},
		{
			name: "tooLongKey",
			text: (() => {
			...
			diagnostics: [{ range: [1, 4, 1, 519], error: keyTooLong }],
		},
		{
			name: "tooLongProp",
			text: { test: "a".repeat(257) },
			diagnostics: [{ range: [1, 12, 1, 271], error: valueTooLong }],
		},
		{
			name: "tooManyProps",
			text: (() => {
	...
	for (const testCase of testCases) {
		test(testCase.name, () => {
			const text =
				typeof testCase.text === "object"
					? JSON.stringify(testCase.text, undefined, 4)
					: String(testCase.text);
			const diagnostics: Diagnostic[] = getTagDiagnostics(text);
			assert.strictEqual(
				diagnostics.length,
				testCase.diagnostics.length,
				"Number of diagnostics does not match."
			);
			for (const actual of diagnostics) {
				const expected = nonNullValue(testCase.diagnostics.shift());
				const [startLine, startChar, endLine, endChar]: number[] =
					expected.range;
				assert.strictEqual(
				...
				assert.strictEqual(
					...
					endChar,
					"End char does not match."
				);
				assert.ok(
					expected.error.test(actual.message),
