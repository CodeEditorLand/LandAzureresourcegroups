...
suite("ResourceGroupGroupingItem", async () => {
	test("Validate element", async () => {
		...
		assert.ok(
			hasViewProperties(resourceGroup),
			"ResourceGroupGroupingItem should have viewable properties"
		);
		assert.ok(
			hasPortalUrl(resourceGroup),
			"ResourceGroupGroupingItem should have a portal URL"
		);
		assert.ok(
			resourceGroup.subscription,
			"ResourceGroupGroupingItem should have a subscription"
		);
